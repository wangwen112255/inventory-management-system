    ...
    <tbody>
        <?php foreach ($lists as $data) { ?>
            <tr>
                ...
                <td>-</td>
                <td class="action">
                    <a class="btn btn-primary btn-sm db-import" href="<?php echo url('import?time=' . $data['time']); ?>"> <i class="iconfont icon-chexiao"></i> 还原</a>&nbsp;
                    <a class="btn btn-danger btn-sm ajax-get confirm" href="<?php echo url('del?time=' . $data['time']); ?>"> <i class="iconfont icon-shanchu"></i> 删除</a>
                </td>
            </tr>
        <?php } ?>
    </tbody>
